...
public final class HiddenBlockStrategyTest extends AbstractStrategyTestCase {

  @Override
  protected Collection<Command> getCommands() {
    final Collection<Command> commands = new ArrayList<Command>();
    commands.add(new RetainCandidatesCommandBuilder(null, 0, 8).addCandidate(1, 9).build());
    commands.add(new RetainCandidatesCommandBuilder(null, 1, 8).addCandidate(1, 9).build());
  ...
  protected Strategy getStrategy() {
    ...
    hiddenBlockStrategy.setSize(2);
    return hiddenBlockStrategy;
  }
